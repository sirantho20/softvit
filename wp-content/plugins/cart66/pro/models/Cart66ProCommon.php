  ...
  public static function showChangelog() {
    if($_REQUEST["plugin"] == "cart66") {
      ...
      if($orderNumber) {
        ...
        $raw = $raw . "h1,h2,h3,li {font-size:20px; font-family:'Lucida Grande',Lucida,'Lucida Sans',Arial,sans-serif;}";
        $raw = $raw . "h2 {font-size:16px;}";
        $raw = $raw . "h3 {font-size:16px;}";
        $raw = $raw . "li {font-size:13px;line-height:150%; margin-bottom:6px;}";
        $raw = $raw . "#update {margin:10px; padding: 20px; border:1px solid #dedede;}";
        $raw = $raw . "</style>";
        $raw = $raw . "<div id='update'>";
        $raw = $raw . "<h1>What's new in Cart66</h1>$matches[1]";
        $raw = $raw . "</div>";
        echo $raw;
      }
