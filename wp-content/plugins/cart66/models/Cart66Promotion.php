...
class Cart66Promotion extends Cart66ModelAbstract {
  ...
  public function save() {
    ...
    if(count($errors) == 0) {
      $this->_data['code'] = strtoupper("," . str_replace(" ","",$this->_data['code']) . ",");
      
      $this->min_order = ($this->min_order == "") ? null : $this->min_order;
      $this->effective_from = (!empty($this->effective_from)) ? date("Y-m-d H:i:s",strtotime($this->effective_from)) : null;
      $this->effective_to =  (!empty($this->effective_to)) ? date("Y-m-d H:i:s",strtotime($this->effective_to)) : null;
        ...
        if(empty($this->products)){
          ...
          foreach($cartItems as $item) {
            for($i=1; $i <= $item->getQuantity(); $i++){
              if(!empty($this->max_uses_per_order) && $counter < ($this->max_uses_per_order)){
                $discount+= $this->stayPositive($item->getProductPrice(),$this->getAmount($item->getProductPrice()));
              }
          ...
          foreach($cartItems as $item) {
            if (in_array($item->getProductId(), $products)) {
              ...
              $productDiscount = $this->getAmount($item->getProductPrice());
              $discount += $allowedQuantity * $this->stayPositive($item->getProductPrice(),$productDiscount);
            }
          }
        }
